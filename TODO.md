...
|---|---|---|---|
| Franco | C-06 + C-07 | `seed-mandates.ts` (parsea brands/*.yaml via loader → INSERT mandates con payload+prompt jsonb + signature mvp:dummy, idempotente por account_id) + `seed-inventory.ts` (3 zonas para streamer-team creator: lower_third / bottom_right_corner / fullscreen_takeover con floors + max_duration, idempotente por creator+zone). Destraba C-08 de Andy. | 2026-05-09 16:40 |
