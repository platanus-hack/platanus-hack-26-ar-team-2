...
| Franco | P0-20 + A-06 | $30 USDC ya depositados de Lemon a owner wallet — falta distribuir $5 USDC + 0.0001 ETH a c/u de las 4 brand wallets | 2026-05-09 15:05 |
...
- ⬜ **A-11** `[INFRA]` **(Andy — preguntale a Franco los valores)** Sincronizar las 4 env vars de runtime del Track A a Vercel (Production + Preview + Development) y re-deployar. Sin esto el deploy actual de prod (`DEPLOY-PROD` ✅ commit `e172596`) tira **500 en cualquier ruta que importe `viem.ts` o `privy.ts`** porque el módulo hace `throw` al cargar si las vars faltan. Vars: `ALCHEMY_RPC_URL` (`apps/web/src/lib/chain/viem.ts:14`), `PRIVY_APP_ID` + `PRIVY_APP_SECRET` (`apps/web/src/lib/chain/privy.ts:47-48`, server-only — el secret nunca al client bundle), `NEXT_PUBLIC_PRIVY_APP_ID` (Privy SDK browser, mismo valor que `PRIVY_APP_ID`). Camino rápido: `cd apps/web && vercel env ls` para auditar qué falta, después `vercel env add <NAME>` por cada una y `vercel --prod` para re-deployar. Cuando esté listo, avisar al team que pueden bajar el set completo con `vercel env pull` desde `apps/web/`. — deps: P0-11, P0-13, DEPLOY-PROD
