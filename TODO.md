...
| Franco | C-09 | Streamer-agent runner **single-shot al deadline T+5s** (variant decidida 2026-05-09 — DESIGN.md §4 actualizado). En `apps/web/src/lib/agents/streamer/streamerEvaluate.ts`: recibe `{ standing_offers[], market_signals, manager_decision, creator_mandate }` y devuelve `NegotiationTurn { from: 'streamer', action: 'accept'|'walk', winner_brand_id?, reason }`. Claude Sonnet 4.6 con prompt es-AR ≤25 palabras + reservation price (RP) override gate (si LLM trata de aceptar bajo dynamic_reserve, override a 'walk'). NO counter-ea turno-a-turno (eso era el plan original — descartado por presupuesto de latencia). Stub picker para `MANAGER_DRY_RUN=true`. Smoke `pnpm smoke:streamer` con fixtures sintéticos sin LLM real. **Lleva el trabajo en worktree `feat/c-09-streamer-agent`**, FF a main al cerrar. | 2026-05-09 |
...
- ⬜ **C-08m-watermark** **[ANALYSIS · post-integración]** Edge case identificado en el flow actual de `apps/web/src/lib/manager/tick.ts`: si el pipeline atrasa chunks (chunk_period > polling cadence) el cron lee el mismo `context_chunks` row varias veces consecutivas y emite varios `render_events kind='brand'` duplicados sobre el mismo chunk. **No se toca hasta cerrar Phase 2 (I-01..I-09 — integración end-to-end).** El razonamiento: hasta que el happy path I-01 esté corriendo en vivo (stream → context → cron → render → escrow), no tenemos baseline de cuánto duplica realmente el cron en condiciones reales — y el shape del manager puede seguir cambiando durante integración, lo que invalidaría cualquier patch prematuro. Tres dimensiones a estudiar cuando se retome: (1) **¿es real en prod?** medir cuántas veces el cron toca el mismo `chunk.id` en 5 min de stream tras I-01; (2) **¿el flow post-integración lo necesita?** con always-emit `"…"` fallback los duplicados son menos visibles que con el flow viejo + Stage1+cooldown; (3) **¿dónde defender?** opciones: schema (`render_events.source_chunk_id` FK + check pre-emit), aplicación (last_emitted_chunk_id en memoria del proceso del cron), o cron config (advisory lock por chunk_id). Drop del intento del 2026-05-09 (Franco) — colisionaba en migration number `0011_*` con `0011_render_events_payload.sql` (Andy/D-02) y el shape de tick.ts cambió bajo el patch. — deps: I-01 (Phase 2 happy path)
...
- 🟡 **C-09** streamer-agent runner **single-shot al deadline T+5s** (`apps/web/src/lib/agents/streamer/streamerEvaluate.ts`): recibe snapshot de standing offers + market_signals + manager_decision + creator mandate, **NO** counter-ea turno a turno. Output: `NegotiationTurn { from: 'streamer', action: 'accept'|'walk', winner_brand_id?, reason: <es-AR ≤25 palabras> }`. Sonnet 4.6 con RP override gate (si LLM trata de aceptar bajo `dynamic_reserve_usdc`, override a 'walk'). Variant decidida 2026-05-09 sobre el plan original "counter-batched per turn" — DESIGN.md §4 actualizado. Stub picker para `MANAGER_DRY_RUN=true`. Smoke `pnpm smoke:streamer` con fixtures sintéticos. — deps: C-01, P0-07
...
- ⬜ **D-10b** **Logos + assets visuales para los 4 merchants ficticios.** Hoy ningún YAML/brand registry tiene `logo_url` cargado y `FallbackAd` solo dibuja la inicial sobre el `color`. Necesitamos por merchant: (a) **logo cuadrado** 512×512 PNG transparente para `bottom_right_corner` + brand console + dock placements, (b) **lockup horizontal** 1920×180 SVG/PNG para `lower_third` (logo + tagline a la izq, espacio para CTA QR a la der), (c) **splash fullscreen** 1920×1080 PNG/MP4 para `fullscreen_takeover` (FULL BREAK) — visualmente el más rico, es el "premium spot" del demo. Drop a `apps/web/public/brands/<slug>/{logo.png,lower-third.png,fullscreen.png}` y agregar `logo_url` + `lockup_url` + `fullscreen_url` a los 4 YAMLs. **Sirve también para reemplazar los placeholders hardcoded de Coca-Cola/Adidas/MP en demo placements (D-10c).** Estimado 2-3 hs con asistencia de imagen IA (Midjourney/Imagen). — deps: ninguna
