...
| Franco | C-08m-watermark | Cierra edge case de re-emit en `apps/web/src/lib/manager/tick.ts`: si el pipeline se atrasa (chunk_period > cooldown), el cron puede leer el mismo `context_chunks` row dos veces post-cooldown y emitir un segundo `render_events kind='brand'` sobre el mismo chunk. Fix: migration `0011_render_events_source_chunk_id.sql` agrega columna nullable + FK + index parcial; `tick.ts` chequea `WHERE source_chunk_id = chunk.id` antes del cooldown (skip determinístico por chunk_id) y graba el `source_chunk_id` en el INSERT del brand emit. Smoke test pure-JS con fake pg client (`scripts/smoke-manager-no-replay.mts`) prueba 3 escenarios: emit-on-fresh-chunk · skip-on-same-chunk-post-cooldown · emit-on-new-chunk. Sin tocar el cron interval ni el resto del flow. | 2026-05-09 18:00 |
