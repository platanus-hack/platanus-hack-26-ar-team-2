...
| Franco | C-08m-pivot-cleanup | Cleanup post-pivote latencia (5-7s → 8-13s) + detección semántica (trigger words → audio_summary): reescribir 4 brand YAMLs a tech-pitch · reescribir `lib/brands.ts` a 4 brands ficticios + `match_keywords` (arregla stub picker bug) · alinear `pickBrand.ts` system prompt + tool description · `OverlayClient.tsx` lee `duration_ms` del payload (fallback 8s) · borrar SCRIPT obsoleto de `DemoDisplayClient.tsx` (Adidas/Nike/MP) · eliminar `apps/manager-worker/` (consolidar en cron variant) · docs (DEMO_RUNBOOK Bloque 3 + on-chain timing + DESIGN §3/§4 + GATES). | 2026-05-09 |
...
- ⬜ **D-10c** **Limpiar referencias a marcas reales en demo fixtures.** El `DockWrapper.tsx` (líneas 7-35) muestra demo placements de Adidas / Coca-Cola / Mercado Pago — incluyendo el `fullscreen_takeover` con `Coca-Cola/premium_takeover` que es el slot del FULL BREAK. Lo mismo en `DemoDisplayClient.tsx` (Adidas / Nike / Mercado Pago). Reemplazar por los 4 fictional merchants — sugerido para FULL BREAK: **CafetITO** (su YAML ya define `premium_takeover` 30s fullscreen) o **TermoFlex** (default always-on, calza como ad institucional). Idem en cualquier otro fixture / mock data del repo. — deps: D-10b
