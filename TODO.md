...
- 🟡 **C-08m-cron** **Variante Vercel Cron del manager** — port de la lógica del worker (Stage1 semantic filter + Stage2 Claude Haiku con scores moment_quality/brand_match) a `apps/web/src/lib/manager/`, expuesta via `GET /api/internal/manager-tick` con Bearer auth (`CRON_SECRET`). `apps/web/vercel.json` dispara la route cada minuto. Cooldown derivado de la última row en `render_events`. Co-existe con `apps/manager-worker/` standalone — el deploy elige cuál correr. Trade-off: 0–60s latency vs <1s del worker push. Útil cuando no querés mantener un host always-on (Fly/Railway). — deps: C-08m
