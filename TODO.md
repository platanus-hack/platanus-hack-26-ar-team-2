...
- ⬜ **C-08m-watermark** **[ANALYSIS]** Edge case identificado en el flow actual de `apps/web/src/lib/manager/tick.ts`: si el pipeline atrasa chunks (chunk_period > polling cadence) el cron lee el mismo `context_chunks` row varias veces consecutivas y emite varios `render_events kind='brand'` duplicados sobre el mismo chunk. Hay tres dimensiones a estudiar antes de patchear: (1) **¿es real en prod?** medir cuántas veces el cron toca el mismo `chunk.id` en 5 min de stream con el cron actual; (2) **¿el flow simplificado actual lo necesita?** post-refactor a always-emit con `"…"` fallback, los duplicados son menos visibles que con el flow viejo + Stage1+cooldown; (3) **¿dónde defender?** opciones: schema (`render_events.source_chunk_id` FK + check pre-emit), aplicación (pasar last_emitted_chunk_id en memoria del proceso del cron), o cron config (advisory lock por chunk_id). Drop del intento del 2026-05-09 (Franco) — colisionaba en migration number `0011_*` con `0011_render_events_payload.sql` (Andy/D-02) y el shape de tick.ts cambió bajo el patch. Re-engager post-MVP cuando el flow esté estable. — deps: ninguna (análisis)
