...
| Franco | C-08a | Gate1 mandate determinístico — pure function `evaluateGate1()` en `apps/web/src/lib/agents/brand/gates/gate1Mandate.ts`, sin LLM. Chequea event_filters (required_any_tag, preferred_categories, min_viewers, max_viewers, required_chat_keyword_any), brand_safety (blocked_keywords, blocked_competitor_brands, blocked_categories), dayparts (con wrap-around past-midnight), budget (daily_cap). Bypass para `always_bid_floor: true` (TermoFlex). Wired al cron flow vía `applyGateLadder()` que pre-filtra brands antes del picker; persiste `gate_skips: GateSkipReason[]` en `render_events.payload`. Calibra `cafetito.min_viewers` 50→2 y `pancho-rex.min_viewers` 30→0 para alinear con PITCH narrative + audiencia demo 5-10. Habilita F-02/F-03/F-05 del harness con `gate_skips` esperados; `sim-orchestrator.check()` verifica subset match. | 2026-05-09 |
| Franco | C-08c | Gate3 Haiku triage — async function `evaluateGate3()` en `apps/web/src/lib/agents/brand/gates/gate3Haiku.ts`. Claude Haiku tool_use con `{should_proceed, ad_id_candidate?, confidence}`. ~150 tok IN / ~50 tok OUT. NO se wirea al cron flow todavía (eso es C-08d cuando lande la per-brand restructure) — queda callable + smoke test independiente (`scripts/smoke-gate3.mts`) que ejercita 2-3 escenarios contra Claude real. Harness corre F-01/F-04 con `MANAGER_DRY_RUN=false` para validar que el LLM no rompe expects deterministas con el ladder activo. | 2026-05-09 |
...
- 🟡 **C-08a** **Gate1 — mandate determinístico** (`apps/web/src/lib/agents/brand/gates/gate1Mandate.ts`). Función pura que recibe `(BrandMandate, StreamMetadata, ContextTick)` → `{ pass: boolean, skip_reason?: GateSkipReason }`. Chequea `event_filters` (required_any_tag, preferred_categories, min_viewers, required_chat_keyword_any), `dayparts.active`, `brand_safety.blocked_keywords` contra `audio_30s/recent_keywords`, `blocked_competitor_brands` contra brands ya pautadas en últimos 5 min. Bypass para brands con `always_bid_floor: true` (skip gate3/4 — gate2 está deferred a post-MVP). Emite `GateSkipReason` con `human_message` es-AR. ~100 LoC, sin LLM. Spec en `docs/GATES.md §2-§3`. **Harness contract (C-08test):** habilitar fixtures F-02 (pancho-rex SKIP daypart), F-03 (matebros SKIP viewers), F-05 (brand-safety SKIP) y verificar `gate_skip_reasons[]` esperados antes de mergear. — deps: C-02b, C-02c
...
- 🟡 **C-08c** **Gate3 — Haiku triage** (`apps/web/src/lib/agents/brand/gates/gate3Haiku.ts`). Llamada barata a Claude Haiku con prompt mínimo: `{mandate.persona, recent_context}` → `{should_proceed: bool, skip_reason?: string}`. ~150 tokens IN, ~50 tokens OUT, ~$0.0008 por call, ~200ms p95. Lo importante es filtrar momentos que pasan gate1 pero no calzan en voice/persona del brand (ej. CafetITO premium en clutch épico que también tiene chat tóxico). En MVP recibe el tick directo de gate1 (gate2 está deferred a post-MVP §14 — cuando lande, el prompt suma `gate2_similarity` como hint). Spec en `docs/GATES.md §2`. **Harness contract (C-08test):** correr F-01..F-06 con `MANAGER_DRY_RUN=false` para validar que el LLM no rompe los expects deterministas; agregar fixture nuevo si Haiku falsea. — deps: C-08a, P0-07
