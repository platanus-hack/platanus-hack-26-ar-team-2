...
| Franco | C-08test | Orchestrator harness con fixtures sintéticos — `apps/web/scripts/fixtures/orchestrator-cases.ts` (6 casos calibrados al PITCH Bloque 3) + `apps/web/scripts/sim-orchestrator.ts` runner que INSERTa chunks via pg + llama `managerTick()` directo + lee `render_events` + comparara contra expected. Bypass de OBS+pipeline → dev-loop de gates/agentes en segundos en lugar de minutos. Primer pase: F-01 cafetito-match + F-04 termoflex-floor enabled (single-brand pick contra `managerTick` actual). F-02/F-03/F-05/F-06 marcados `skip:true` con TODO refs a C-08a/C-08b/C-08c/C-08d/C-14 — cuando esos lleguen se enabled + se enriquece el shape de expectativas. Trigger del runner switchea de `managerTick()` directo a `POST /api/auctions/run` cuando C-14 exista. **HEADS UP TRACK C**: cualquier cambio en gates/orchestrator agrega o actualiza un fixture acá antes de mergear — el harness es el contrato. | 2026-05-09 |
...
- 🟡 **C-08test** **Orchestrator harness con fixtures sintéticos** (`apps/web/scripts/sim-orchestrator.ts` + `apps/web/scripts/fixtures/orchestrator-cases.ts`). Permite validar el orquestador (manager + gates + agentes + settlement) sin levantar OBS/RTMP/pipeline: el script INSERTa una row sintética en `context_chunks`, dispara `managerTick()` directo (hoy) o `POST /api/auctions/run` (cuando C-14 exista), y comparará el `render_events` resultante contra `expected`. Casos calibrados al PITCH Bloque 3 (`docs/PITCH.md`): F-01 cafetito-match (mention "café"), F-02 pancho-rex SKIP daypart, F-03 matebros SKIP viewers, F-04 termoflex floor-fill, F-05 brand-safety SKIP, F-06 cafetito-vs-termoflex negociación. **MVP shape: F-01 + F-04 enabled, los demás `skip:true` con TODO ref a la task que los desbloquea.** Heads up para Track C: cualquier PR que toque gates/orchestrator agrega/updates un fixture antes de mergear. Cuando C-08a/b/c/d landeen, se enriquece el `expected` con `gate_skip_reasons[]`; cuando C-14 lande, el trigger del runner cambia de import directo a HTTP POST. — deps: ninguna (corre contra el código actual)
