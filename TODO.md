...
- 🟡 **C-02d** **Mandates calibrados al PITCH Bloque 3** (✅): CafetITO daypart `11:00-23:59` (matchea demo a 12:00), Pancho Rex daypart `13:00-15:00 + 20:00-02:00` (skip a 12:00 = "no es lunch"), MateBros `max_viewers: 80` + `required_any_tag` sin high_energy (skip primer trigger por viewers o tag mismatch, match segundo por casual_chat), TermoFlex 24/7 always_bid_floor. **Pendiente del C-02d (🟡):** confirmar canal Twitch del equipo y viewer count real → ajustar `streams/demo.yaml` y `matebros.max_viewers` antes del demo. — deps: C-02b ✅, C-02c ✅, PD-07b ⬜
