...
- ⬜ **C-13a** **Event broadcast foundation** — pattern reusable de "API POST → Supabase Realtime broadcast → page subscriber". Implementar `POST /api/banner/broadcast { message, channel?, ttl_ms? }` (usa Supabase Realtime broadcast REST endpoint, server-side con service_role) + `/banner` page (Client Component subscribe a `banner:<channel>` con supabase-js). Esto es la base que C-14 reusa para emitir `auction:<stream_id>:settled`, `placement:<stream_id>`, etc. Smoke-test: `curl -X POST .../api/banner/broadcast -d '{"message":"Hola"}'` → banner page muestra "Hola". — deps: P0-12
- ⬜ **C-14** Endpoint `POST /api/auctions/run` que recibe `{ tick, manager_decision }` del manager-worker y corre la subasta sincrónica (~5-8s): `computeMarketSignals(tick)` → 8 brand-agents `huntForBrand()` paralelo → orchestrator multi-turno con AC_combi + curva de concesión → `pickWinner()` → INSERT placements → `escrow.lock()` → broadcast `auction:<stream_id>:settled` + `placement:<stream_id>` por Supabase Realtime (ver DESIGN.md §4 Event flow, reusa pattern de C-13a). Durante la subasta, broadcast `auction:<auction_id>:turn` por cada turno para el demo display. — deps: C-10, C-12, C-13a
