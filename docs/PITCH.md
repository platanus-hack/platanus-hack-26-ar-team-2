...
| **"Y mirá lo que pasa cuando estamos charlando de algo concreto — yo me agarro este café"** *(speaker levanta el café, lo muestra a cámara unos segundos)* **"y le explico al jurado que esto es el comeback del streamer chico, que venía perdiendo plata contra el pre-roll y ahora gana en cada momento de la charla."** | El pipeline corre B-07c: extrae `audio_mentions: ["café"]`, `audio_topics: ["bebida","streaming","monetización"]`, mood `high_energy` por la palabra "comeback" + cadencia de explicación. Dispara auction. |
| *(mientras el sistema procesa — hasta ~10s — el operador del dashboard hace zoom-in al log de gates, que se actualiza en <1s, para llenar la pausa visual)* | Log: <br>`☕ CafetITO → MATCH (gate1 ✓ mention "café" + mood comeback ∈ target_moods)` <br>`🧊 TermoFlex → MATCH (always_bid_floor, gate2/3/4 bypass)` <br>`🌭 Pancho Rex → SKIP gate1: daypart no activo (no es lunch)` <br>`🧉 MateBros → SKIP gate1: viewer count > max_viewers íntimo` |
